...
class XMLGenerator:
    ...
    def __init__(
        ...
        self.model_created = model.get('created', '2025-01-01T00:00:00.0000000Z')
    ...
    def generate(
        ...
        Args:
            paths (list): Список путей для создания объектов.
            external_children (dict): Внешние дети: parent_path → [uid].
            parent_uid (str): UID корневого родителя.
            cck_map (dict): Карта: path → ККС-код.
            parent_uid_map (dict): Карта: child_path → parent_uid (виртуальный).
            virtual_containers (set, optional): Множество виртуальных путей.

        Returns:
            str: Готовый RDF/XML как строка.

        Raises:
            ValueError: Если нет данных для генерации.
        """
